...
build-image: build-all
	...
	podman build -t ${DOCKER_PREFIX}/${IMAGE_NAME}:${DOCKER_TAG} -f _output/bin/$(GOOS)/$(GOARCH)/Dockerfile _output/bin/$(GOOS)/$(GOARCH)

push: build-image
	@echo -e "${GREEN}Pushing plugin image to local registry${WHITE}"
	@podman push ${DOCKER_PREFIX}/${IMAGE_NAME}:${DOCKER_TAG}
