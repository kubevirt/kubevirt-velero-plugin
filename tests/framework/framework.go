...
func UpdateVMStateStorageClass(kvClient kubecli.KubevirtClient) {
	...
	_, err = kvClient.KubeVirt(kv.Namespace).Patch(kv.Name, types.JSONPatchType, []byte(patchData), &metav1.PatchOptions{})
	gomega.Expect(err).ToNot(gomega.HaveOccurred())
}
...
func GetKubevirt(kvClient kubecli.KubevirtClient) *kv1.KubeVirt {
	...
	gomega.Eventually(func() error {
		kvList, err = kvClient.KubeVirt(metav1.NamespaceAll).List(&metav1.ListOptions{})
